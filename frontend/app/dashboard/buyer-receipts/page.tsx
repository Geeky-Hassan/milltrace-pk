...
import { createBuyerReceipt, getBuyerReceipts } from "@/lib/api";
...
import type { BuyerReceipt } from "@/types";
        ...
        <form onSubmit={handleSubmit} className="mb-6 rounded-lg border border-ink-100 bg-white p-5 shadow-soft">
          ...
          <div className="mt-5 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
            <label className="grid gap-1.5 text-sm font-semibold text-ink-700">
              Dispatch ID
              <input required value={form.dispatch_id} onChange={(event) => setForm({ ...form, dispatch_id: event.target.value })} className="h-10 rounded-md border border-ink-200 px-3 outline-none focus:border-compliance-green focus:ring-2 focus:ring-emerald-100" />
            </label>
            <label className="grid gap-1.5 text-sm font-semibold text-ink-700">
              Buyer name
