...
export default function DispatchPage() {
  ...
  const [form, setForm] = useState({ buyer: "", vehicle_number: "", driver_name: "", serial_numbers: "" });
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setForm({ buyer: "", vehicle_number: "", driver_name: "", serial_numbers: "" });
        ...
        <form onSubmit={handleSubmit} className="mb-6 rounded-lg border border-ink-100 bg-white p-5 shadow-soft">
          ...
          <div className="mt-5 grid gap-4 md:grid-cols-2 xl:grid-cols-5">
            ...
            <div className="rounded-md border border-emerald-100 bg-emerald-50 px-3 py-2 text-sm">
              <p className="font-bold text-compliance-green">Auto invoice</p>
              <p className="mt-1 text-xs text-emerald-700">Generated by backend on dispatch creation</p>
              <p className="mt-1 text-xs text-ink-500">Future integration: digital invoicing platform.</p>
            </div>
