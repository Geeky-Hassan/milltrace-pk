...
class DemoScenarioService:
    ...
    def trace_batch(self, batch_id: str) -> BatchTraceRead:
        ...
        warehoused_count = sum(row.quantity for row in warehouse_receipts)
        dispatched_count = sum(dispatch.quantity for dispatch in dispatches)
        received_count = sum(receipt.received_quantity for receipt in receipts)
        expected_bags = int(batch.actual_sugar_output_kg // 50)
        cane_weight = sum(item.net_cane_weight_kg for item in cane_intakes)
        warehouse_locations = ", ".join(sorted({row.warehouse_location for row in warehouse_receipts}))
        dispatch_evidence = ", ".join(
            f"{dispatch.dispatch_id} / {dispatch.invoice_number or 'missing invoice'} / {dispatch.quantity} bags"
            for dispatch in dispatches
        )
        receipt_evidence = ", ".join(
            f"{receipt.dispatch.dispatch_id}: {receipt.received_quantity} bags, {receipt.shortage_mismatch}"
            for receipt in receipts
            if receipt.dispatch
        )
        exception_evidence = ", ".join(f"{alert.type}: {alert.description}" for alert in exceptions[:3])

        steps = [
            TraceStep(stage="Cane Intake Records", status="Linked" if cane_intakes else "Missing", timestamp=cane_intakes[0].mill_gate_timestamp if cane_intakes else None, actor=cane_intakes[0].operator_name if cane_intakes else "Operator", evidence=f"{len(cane_intakes)} intake(s), {cane_weight / 1000:,.2f} tons net cane from {', '.join(item.delivery_id for item in cane_intakes) or 'no delivery ID'}", reason="Gross weight must be greater than tare weight, and every accepted intake must link to exactly one production batch.", related_exceptions=self._exception_titles(exceptions, "CaneIntake"), audit_hash=self._audit_hash(matching_audits, "CaneIntake")),
            TraceStep(stage="Production Batch", status=batch.variance_status, timestamp=batch.created_at, actor="Mill Operator", evidence=f"{batch.actual_sugar_output_kg / 1000:,.2f} tons actual sugar vs {batch.expected_sugar_output_kg / 1000:,.2f} tons expected; variance {batch.variance_kg / 1000:+,.2f} tons at {batch.recovery_percentage:.2f}% recovery.", reason="Expected output is calculated from cane input x expected recovery. Warning starts beyond +/-3%; critical starts beyond +/-7%.", related_exceptions=self._exception_titles(exceptions, batch.batch_id), audit_hash=self._audit_hash(matching_audits, batch.batch_id)),
            TraceStep(stage="Generated Serials", status=f"{len(serials)} issued", timestamp=serials[0].timestamp if serials else None, actor="Mill Operator", evidence=f"{len(serials)} issued of {expected_bags} expected 50 kg bag serials; range {serial_range([serial.serial_number for serial in serials]) if serials else 'none'}", reason="Bag quantity is derived from actual sugar output divided by fixed 50 kg bag size. Sequence gaps or duplicate numbers create exceptions.", related_exceptions=self._exception_titles(exceptions, "SERIAL"), audit_hash=self._audit_hash(matching_audits, "GENERATE_SERIALS")),
            TraceStep(stage="Activated Serials", status=f"{activated_count} activated or beyond", timestamp=max((serial.status_updated_at for serial in serials), default=None), actor="Mill Operator", evidence=f"{activated_count} of {len(serials)} serials moved beyond ISSUED; remaining {max(len(serials) - activated_count, 0)} still not activated.", reason="Only ISSUED serials can activate. Activation before lower sequences or any backward movement is flagged.", related_exceptions=self._exception_titles(exceptions, "ACTIVATED"), audit_hash=self._audit_hash(matching_audits, "ACTIVATE")),
            TraceStep(stage="Warehouse Receipt", status=f"{warehoused_count} bags", timestamp=warehouse_receipts[0].received_at if warehouse_receipts else None, actor="Warehouse Manager", evidence=f"{warehoused_count} bags / {warehoused_count * 50 / 1000:,.2f} tons received at {warehouse_locations or 'no warehouse location'}", reason="Only ACTIVATED serials can enter warehouse. Activated serials left outside warehouse beyond 24 hours become leakage risk.", related_exceptions=self._exception_titles(exceptions, "Warehouse"), audit_hash=self._audit_hash(matching_audits, "WarehouseReceipt")),
            TraceStep(stage="Dispatch", status=f"{len(dispatches)} dispatch(es)", timestamp=dispatches[0].dispatched_at if dispatches else None, actor="Warehouse Manager", evidence=dispatch_evidence or "No dispatch has used this batch's serials yet.", reason="Only WAREHOUSED serials can be dispatched, and dispatch quantity must match scanned serial count with invoice evidence.", related_exceptions=self._exception_titles(exceptions, "Dispatch"), audit_hash=self._audit_hash(matching_audits, "Dispatch")),
            TraceStep(stage="Buyer Receipt", status=f"{received_count} bags received", timestamp=receipts[0].receipt_timestamp if receipts else None, actor="Warehouse Manager / Buyer", evidence=receipt_evidence or "Receipt pending; buyer has not confirmed these dispatch serials.", reason="Buyer receipt must match dispatch buyer and serial list. Shortage, extra serials, wrong buyer, or duplicate receipt are flagged.", related_exceptions=self._exception_titles(exceptions, "RECEIPT"), audit_hash=self._audit_hash(matching_audits, "BuyerReceipt")),
            TraceStep(stage="Exceptions", status=f"{len(exceptions)} related", timestamp=exceptions[0].created_at if exceptions else None, actor="Exception Engine", evidence=exception_evidence or "No related exceptions found for this batch, dispatch, or serial set.", reason="Related alerts are grouped from the batch ID, its serials, and dispatch IDs so reviewers see the connected risk picture.", related_exceptions=[alert.title for alert in exceptions[:6]], audit_hash=self._audit_hash(matching_audits, "EXCEPTION")),
            TraceStep(stage="Audit Trail", status=f"{len(matching_audits)} events", timestamp=matching_audits[0].created_at if matching_audits else None, actor="System", evidence="Hash-chain event evidence is retained for review; each event stores current and previous hashes where available.", reason="Audit logs are append-only evidence for sensitive actions such as intake, production, serial movement, dispatch, receipt, and exception handling.", related_exceptions=[], audit_hash=matching_audits[0].event_hash if matching_audits else None),
        ]
        return BatchTraceRead(batch_id=batch.batch_id, summary=f"{cane_weight / 1000:,.2f} tons cane, {batch.actual_sugar_output_kg / 1000:,.2f} tons sugar, {len(serials)} issued serials, {len(exceptions)} related exception(s).", steps=steps)
